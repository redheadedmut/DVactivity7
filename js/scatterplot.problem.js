                                 ...
                                 margin = 50)
{
    ...
    const activeCategories = new Set(); // Track active legend filters
...
// Initialize active categories
    ...
    legends_items.append("rect")
        ...
        .attr("width", "40")
        .attr("height", "40")
        .attr("class", d => d) // Assign class based on category
        ...
        .on("click", function (event, category) {
            if(event.target){
                ...
                if (isActive) { activeCategories.delete(category);
                } else {activeCategories.add(category);}
    
                // Select all rectangles with the same class and update opacity
                d3.selectAll(`rect.${category}`)
                    .attr("opacity", isActive ? 1 : 0.2); // Dim if active, highlight otherwise
    
                // Update circle nodes based on active categories
                d3.selectAll("circle").attr("opacity", d => {
                    const match = activeCategories.has(d[colorCol]);
                    return match ? 0.2 : 1; // Highlight matched, dim others
                });
            }
